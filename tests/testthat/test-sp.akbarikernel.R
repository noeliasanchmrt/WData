test_that("sp.akbarikernel() returns a valid estimate", {
  result <- sp.akbarikernel(biased_models[[1]], bw = 0.1, plot = FALSE)
...
test_that("sp.akbarikernel() correctly handles different kernels", {
  results <- lapply(kernels, function(k) {
    sp.akbarikernel(biased_models[[1]], kernel = k, bw = 0.1, plot = FALSE)
  })
...
test_that("sp.akbarikernel() produces stable plots", {
  skip_on_os(os = c("windows", "linux"))

  lapply(seq_along(biased_models), function(i) {
    lapply(kernels, function(k) {
      vdiffr::expect_doppelganger(
        paste0("sp.akbarikernel_model_", i, "_kernel_", k),
        function() {
          sp.akbarikernel(biased_models[[i]], bw = 0.1, kernel = k, plot = TRUE)
